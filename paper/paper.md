...
Addressing the second challenge requires precise spatial delineation of cells—making **cell segmentation** a critical first step. Segmentation quality directly impacts the accuracy of all downstream analyses. Our current workflow integrates native segmentation approaches: CosMx uses a **Cellpose-based** method, and Xenium applies a **Voronoi expansion** strategy. However, users can also opt for alternative or custom segmentation tools, which may offer better performance but require careful parameter tuning—often difficult to implement in tools like **Xenium Ranger** (10x Genomics) or **AtoMx** (NanoString).
